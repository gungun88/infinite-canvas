...
func New() *gin.Engine {
	...
	api.POST("/auth/register", middleware.MaxBodyBytes(8<<10), middleware.RateLimit(time.Hour, 5), gin.WrapF(handler.Register))
	api.POST("/auth/login", middleware.MaxBodyBytes(8<<10), middleware.RateLimit(time.Minute, 10), gin.WrapF(handler.Login))
	api.POST("/auth/verification/resend", middleware.MaxBodyBytes(8<<10), middleware.RateLimit(time.Hour, 5), gin.WrapF(handler.ResendVerificationEmail))
	api.POST("/auth/forgot-password", middleware.MaxBodyBytes(8<<10), middleware.RateLimit(time.Hour, 5), gin.WrapF(handler.ForgotPassword))
	api.POST("/auth/verify-email", gin.WrapF(handler.VerifyEmail))
	...
	api.GET("/auth/doingfb/authorize", gin.WrapF(handler.DoingFBAuthorize))
	api.GET("/auth/doingfb/callback", gin.WrapF(handler.DoingFBCallback))
	...
	api.POST("/ai/direct-request", middleware.MaxBodyBytes(8<<20), gin.WrapF(handler.PrepareDirectAIRequest))
	...
	v1.POST("/images/generations", middleware.MaxBodyBytes(8<<20), gin.WrapF(handler.AIImagesGenerations))
	v1.POST("/images/edits", middleware.MaxBodyBytes(8<<20), gin.WrapF(handler.AIImagesEdits))
	v1.POST("/responses", middleware.MaxBodyBytes(8<<20), gin.WrapF(handler.AIResponses))
	v1.POST("/chat/completions", middleware.MaxBodyBytes(8<<20), gin.WrapF(handler.AIChatCompletions))
	v1.POST("/models", gin.WrapF(handler.AIModels))
	v1.POST("/audio/speech", middleware.MaxBodyBytes(8<<20), gin.WrapF(handler.AIAudioSpeech))
	...
	v1.POST("/user-config/model", middleware.MaxBodyBytes(4<<20), gin.WrapF(handler.SaveUserModelConfig))
	v1.POST("/user-config/storage", middleware.MaxBodyBytes(64<<10), gin.WrapF(handler.SaveUserStorageProvider))
	v1.GET("/canvas/projects", gin.WrapF(handler.UserCanvasProjects))
	v1.POST("/canvas/projects", middleware.MaxBodyBytes(4<<20), gin.WrapF(handler.SaveUserCanvasProject))
	v1.POST("/canvas/projects/sync", middleware.MaxBodyBytes(8<<20), gin.WrapF(handler.SyncUserCanvasProjects))
	...
	v1.POST("/generation-logs/videos", middleware.MaxBodyBytes(8<<20), gin.WrapF(handler.SaveUserVideoGenerationLogs))
	...
	v1.POST("/generation-logs/images", middleware.MaxBodyBytes(8<<20), gin.WrapF(handler.SaveUserImageGenerationLogs))
	...
	v1.POST("/user-data/assets", middleware.MaxBodyBytes(4<<20), gin.WrapF(handler.SaveUserAssetData))
	...
	api.POST("/admin/login", middleware.MaxBodyBytes(8<<10), middleware.RateLimit(time.Minute, 10), gin.WrapF(handler.AdminLogin))
