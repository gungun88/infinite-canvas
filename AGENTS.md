...
- 本地渠道拉取模型列表必须通过本站后端代理接口，例如 `/api/v1/models`；不要在浏览器端直接请求外部渠道 Base URL 的 `/models`，避免 CORS、密钥暴露和上游兼容问题。
...
- 新增渠道下拉框是本地定制，默认只显示 DoingAI、OpenAI、Gemini 和自定义；上游合并时必须保留 `web/src/lib/custom-channel-policy.ts` 及其引用，不得恢复为全量上游协议列表。
