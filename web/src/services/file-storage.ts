...
async function uploadMediaBlobToServer(blob: Blob, filename: string, metadataUrl?: string): Promise<UploadedFile> {
    ...
    const meta = metadataUrl || payload.data.mimeType?.startsWith("video/") ? await readVideoMeta(metadataUrl || resolvedUrl) : {};
...
export async function resolveMediaUrl(storageKey?: string, fallback = "") {
    ...
    if (storageKey.startsWith("server:")) {
        ...
        if (fallback && !fallback.startsWith("blob:") && !fallback.includes("direct=1") && !fallback.includes("/api/files/") && !fallback.startsWith("/webdav-media/")) return fallback;
